...
export default function AgentCard({
  ...
  return (
    ...
    >
      ...
      <div class="flex items-center justify-between gap-2 border-t border-gray-100 bg-gray-50/60 px-4 py-3">
        <div class="flex items-center gap-1">
          {(['start_work', 'stop_work'] as SwarmWorkCommand[]).map((cmd) => (
            ...
            >
              {cmd === 'start_work' ? (
                <svg class="w-3.5 h-3.5" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /></svg>
              ) : (
                <svg class="w-3.5 h-3.5" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
              )}
            </button>
          ))}
          <button
      ...
      <AgentConfigModal agentId={agent.id} open={configOpen} onClose={() => setConfigOpen(false)} />
    </div>
  );
}
