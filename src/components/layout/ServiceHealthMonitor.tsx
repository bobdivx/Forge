...
interface Container { Names: string; State: string; Status: string; Image: string; }
...
export default function ServiceHealthMonitor() {
  ...
  if (loading) return (
    <div class="px-4 py-6 border-t border-slate-800 animate-pulse">
      <div class="h-3 bg-slate-800 rounded w-1/3 mb-4" />
      <div class="space-y-3">{[1, 2, 3].map((i) => <div key={i} class="flex justify-between items-center"><div class="w-1/2 h-2 bg-slate-800 rounded" /><div class="w-12 h-3 bg-slate-800 rounded-full" /></div>)}</div>
    </div>
  );
  ...
  return (
    <div class="px-4 py-6 border-t border-slate-800">
      ...
      <div class="space-y-3">
        ...
        {containers.length > 6 && <div class="text-[9px] text-center text-slate-600 italic pt-1">+ {containers.length - 6} autres</div>}
      </div>
    </div>
  );
}
