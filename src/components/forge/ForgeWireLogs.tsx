...
export default function ForgeWireLogs() {
  ...
  useEffect(() => {
    let cancelled = false;
    const load = () => {
      fetch('/api/forge-activity')
        .then((res) => res.json())
        .then((data) => {
          if (cancelled) return;
          logForge('GET /api/forge-activity', {
            ok: Boolean(data.ok),
            lineCount: Array.isArray(data.lines) ? data.lines.length : 0,
            ...(data.zimaosDebug && typeof data.zimaosDebug === 'object' ? data.zimaosDebug : {}),
          });
          if (data.lines && Array.isArray(data.lines)) { setLines(data.lines); setErr(null); }
          else { setLines([]); setErr('Réponse inattendue'); }
          setLoading(false);
        })
        .catch(() => { if (!cancelled) { setErr('Flux indisponible'); setLoading(false); } });
    };
    load();
  ...
  const display = loading && lines.length === 0 ? ['[ZIMAOS] Chargement des sessions…'] : lines;
    ...
    <div class="bg-slate-950 rounded-xl border border-slate-800 shadow-xl overflow-hidden flex flex-col h-[300px]">
      <div class="flex items-center justify-between px-4 py-2.5 bg-slate-900 border-b border-slate-800">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-xs font-semibold tracking-wider text-slate-300 uppercase">The Wire — ZimaOS (réel)</h2>
        </div>
        {err && <span class="text-[10px] text-amber-500/90">{err}</span>}
      </div>
      <div class="flex-1 p-4 font-mono text-[11px] overflow-y-auto bg-[#0A0D14] text-slate-300 flex flex-col gap-1.5">
        {display.map((line, i) => {
          let colorClass = 'text-slate-400';
          if (line.includes('[ZIMAOS]')) colorClass = 'text-blue-400';
          ...
          return (
            <div key={i} class="flex items-start gap-3 hover:bg-slate-800/50 px-2 py-0.5 rounded transition">
              <span class="text-slate-600 shrink-0 select-none">{new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
