...
export default function AppLauncher() {
  ...
  const createApp = async () => {
    if (!name.trim()) { setStatus('Nom invalide'); return; }
    setLoading(true);
    ...
    try {
      ...
      if (!res.ok) { setStatus(data.error || 'Erreur lors de la création'); }
      else { setStatus(`Ordre envoyé ! L'agent CHEF_TECHNIQUE s'occupe de ${data.app}.`); setName(''); }
    } catch {
    ...
    <div class="relative z-10 flex flex-col h-full">
      ...
      <div class="mt-auto">
        ...
        <button
          ...
          class={`w-full py-2.5 rounded-lg font-semibold text-sm transition shadow-lg flex items-center justify-center gap-2 ${loading || !name.trim() ? 'bg-slate-800 text-slate-500 cursor-not-allowed shadow-none border border-slate-700' : 'bg-blue-600 text-white hover:bg-blue-500 shadow-blue-500/20'}`}
        >
          {loading ? (
