...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!repoUrl || !repoName) {
      return new Response(
        JSON.stringify({ error: "repoUrl et repoName requis" }),
        { status: 400 },
      );
    }

    if (!/^[a-zA-Z0-9_.-]+$/.test(repoName)) {
      return new Response(JSON.stringify({ error: "Nom de dépôt invalide" }), {
        status: 400,
      });
    }
    ...
    if (!githubToken || githubToken.trim() === "") {
      return new Response(JSON.stringify({ error: "Jeton GitHub manquant" }), {
        status: 400,
      });
    }
    ...
    if (fs.existsSync(targetPath)) {
      return new Response(
        JSON.stringify({
          error: "Un dossier avec ce nom existe déjà dans Forge.",
        }),
        { status: 400 },
      );
    }
    ...
    const authUrl = repoUrl.replace(
      "https://",
      `https://oauth2:${githubToken}@`,
    );

    // Clone the repository
    const { stdout, stderr } = await execFilePromise(
      "git",
      ["clone", "--", authUrl, repoName],
      { cwd: reposRoot },
    );
    ...
    try {
      const syncScript = path.join(
        process.cwd(),
        "src/pages/api/sync-projects.ts",
      );
    ...
    return new Response(
      JSON.stringify({
        status: "ok",
        message:
          'Dépôt cloné avec succès ! Cliquez sur "Synchroniser" pour l\'ajouter au tableau de bord.',
      }),
      {
        status: 200,
    ...
    const errorMessage = (error.message || "Erreur lors du clonage").replace(
      /https:\/\/[^@]+@/g,
      "https://***@",
    );
