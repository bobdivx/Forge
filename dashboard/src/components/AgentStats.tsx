...
export default function AgentStats() {
  ...
  return (
    <div class="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-full max-h-[400px]">
      ...
      {error && (
        <div class="px-5 pt-3 text-[11px] text-amber-400 border-b border-slate-800 pb-2">{error}</div>
      )}
            ...
            <div class="flex items-center justify-between group" key={agent.id || agent.name}>
              <div class="flex items-center gap-2.5 min-w-0">
                <div class="relative flex h-2 w-2 shrink-0">
                  <span
                    class={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${agent.status === 'actif' ? 'bg-emerald-400' : 'bg-slate-400'}`}
                  ></span>
                  <span
                    class={`relative inline-flex rounded-full h-2 w-2 ${agent.status === 'actif' ? 'bg-emerald-500' : 'bg-slate-500'}`}
                  ></span>
                </div>
                <span class="text-xs font-mono text-slate-300 group-hover:text-white transition cursor-default truncate">
                  {agent.name}
                </span>
              </div>
              <div class="flex flex-col items-end gap-0.5 shrink-0 ml-2">
                <span class="text-[10px] font-mono text-slate-500 max-w-[9rem] truncate" title={agent.model}>
                  {agent.model}
                </span>
                <span
                  class={`text-[10px] uppercase font-semibold tracking-wider px-2 py-0.5 rounded border ${agent.status === 'actif' ? 'text-emerald-500 bg-slate-950 border-emerald-500/20' : 'text-slate-500 bg-slate-950 border-slate-800'}`}
                >
