#!/bin/bash
# configure_zimaos_default_agent.sh
#
# Configure acp.defaultAgent dans les settings ZimaOS.
# À exécuter sur le ZimaCube (ou via docker exec).
#
# Usage:
#   ./configure_zimaos_default_agent.sh [agentId]
#   ./configure_zimaos_default_agent.sh DEV_FRONTEND   (défaut si omis)

set -euo pipefail

DEFAULT_AGENT="${1:-DEV_FRONTEND}"
ZIMAOS_DATA="${ZIMAOS_DATA:-/DATA/AppData/zimaos}"
...
echo "→ Répertoire ZimaOS    : $ZIMAOS_DATA"

# ── Recherche du fichier settings ──────────────────────────────────────────────
# Tenter d'abord les emplacements connus, puis chercher plus largement.
SETTINGS_CANDIDATES=(
  "$ZIMAOS_DATA/settings.json"
  "$ZIMAOS_DATA/config/settings.json"
  "$ZIMAOS_DATA/agents/main/agent/settings.json"
  "$ZIMAOS_DATA/workspace/settings.json"
)
...
# Fallback : chercher le premier settings.json dans l'arbo ZimaOS
if [[ -z "$SETTINGS_FILE" ]]; then
  SETTINGS_FILE=$(find "$ZIMAOS_DATA" -name "settings.json" -maxdepth 5 2>/dev/null | head -n 1 || true)
...
if [[ -n "$SETTINGS_FILE" ]]; then
  ...
  SETTINGS_DIR="$ZIMAOS_DATA/config"
...
echo "─────────────────────────────────────────────────────"
echo ""
echo "→ Redémarrez le container ZimaOS pour appliquer :"
echo "   docker restart zimaos"
echo ""
