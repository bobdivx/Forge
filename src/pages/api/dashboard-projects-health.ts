...
export const GET: APIRoute = async ({ locals }) => {
  ...
  // Initiate slow external network call early
  const zimaOSSessionsPromise = fetchZimaOSSessionsPayload(email);
  zimaOSSessionsPromise.catch(() => {});
  ...
  try {
    ...
    const [projects, tasksAll, workScheduler] = await Promise.all([
  ...
  try {
    const oc = await zimaOSSessionsPromise;
