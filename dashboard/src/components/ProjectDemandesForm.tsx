    ...
    <section class="bg-slate-900 border border-slate-800 rounded-xl p-6">
      ...
      {!open ? (
        <div class="space-y-3">
          <button
            type="button"
            class="btn btn-sm w-full bg-blue-600 hover:bg-blue-500 border-none text-white"
          ...
          <button
            type="button"
            class="btn btn-sm w-full btn-outline border-slate-700 text-slate-200 hover:bg-slate-800"
            onClick={() => setOpen('fix')}
          >
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button type="button" class="btn btn-ghost btn-xs" onClick={() => setOpen(null)}>
              Fermer
            </button>
          </div>
          <input
            type="text"
            class="w-full bg-slate-950 border border-slate-700 rounded-md px-3 py-2 text-sm text-white"
            placeholder="Titre court"
            value={title}
            onInput={(e) => setTitle((e.target as HTMLInputElement).value)}
          />
          <textarea
            class="w-full bg-slate-950 border border-slate-700 rounded-md px-3 py-2 text-sm text-white min-h-[72px]"
          ...
          <button
            type="button"
            class={`btn btn-sm w-full btn-primary ${saving ? 'loading' : ''}`}
      ...
      {message && <p class="text-xs mt-3 text-emerald-400">{message}</p>}
    </section>
  );
}
