...
export default function ZimaOSLayoutBadge() {
  ...
  useEffect(() => {
    const load = () => {
      ...
      fetch('/api/zimaos-health', { signal: ctrl.signal })
        .then((r) => r.json())
        .then((d) => {
          ...
          if (d.zimaosDebug && typeof d.zimaosDebug === 'object') {
            logForgeZimaOS('GET /api/zimaos-health', {
  ...
  if (ok === null) {
    return <span class={`${badgeBase} badge-ghost text-slate-400`}>ZimaOS…</span>;
  }
  if (!ok) {
    return (
      <span
      ...
      >
        ...
        <span class="hidden sm:inline">ZimaOS indispo</span>
      </span>
    );
  }
  return (
    <span class={`${badgeBase} badge-success text-emerald-100/95`} title="Gateway ZimaOS joignable">
      <span class="status status-success shrink-0 scale-90 animate-pulse" />
      <span class="truncate whitespace-nowrap">
        <span class="hidden min-[400px]:inline">ZimaOS </span>OK
      </span>
    </span>
  );
}
