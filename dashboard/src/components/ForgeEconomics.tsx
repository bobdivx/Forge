...
export default function ForgeEconomics() {
  ...
  const fetchStats = async () => {
    try {
      ...
      console.error("Failed to fetch economics stats", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const cleanSessions = async () => {
    if (confirm('Voulez-vous vraiment fermer les sessions en erreur ou trop anciennes ?')) {
      setCleaning(true);
      try {
    ...
    <div class="bg-gradient-to-br from-slate-900 to-indigo-950 border border-indigo-500/30 rounded-xl p-6 relative overflow-hidden group shadow-xl">
      ...
      <div class="flex justify-between items-start relative z-10">
        ...
        <button 
          ...
          disabled={cleaning || failedSessions === 0}
          class={`btn btn-xs rounded-lg px-3 py-1.5 h-auto text-[10px] font-bold uppercase tracking-tighter border-none 
            ${failedSessions > 0 ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' : 'bg-slate-800 text-slate-500 cursor-not-allowed'}`}
        >
          {cleaning ? <span class="loading loading-spinner loading-xs"></span> : 'Clean Dead Sessions'}
        </button>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4 mt-6 pt-4 border-t border-white/5 relative z-10">
        <div class="space-y-1">
          <div class="flex items-center gap-1.5">
            ...
            <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Actives</span>
          </div>
        ...
        <div class="space-y-1">
          <div class="flex items-center gap-1.5">
            <span class={`w-1.5 h-1.5 rounded-full ${failedSessions > 0 ? 'bg-red-500 animate-pulse' : 'bg-slate-700'}`}></span>
            <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Échecs</span>
          </div>
        ...
        <div class="col-span-2 space-y-1 mt-2">
          ...
          <div class="w-full bg-slate-800 rounded-full h-1 mt-1 overflow-hidden">
            <div class="bg-indigo-500 h-full" style={`width: ${Math.min(100, (totalTokens / 50000) * 100)}%`}></div>
          </div>
        </div>
      </div>
    </div>
