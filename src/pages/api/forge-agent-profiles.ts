...
/**
 * Fiches persistantes pour les **sessions ZimaOS** (clé = `sessionKey` du gateway).
...
export const GET: APIRoute = async ({ locals }) => {
  ...
  try {
    ...
    const rows = await db.select().from(ZimaOSAgentProfile);
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    const existing = await db
      .select()
      .from(ZimaOSAgentProfile)
      .where(eq(ZimaOSAgentProfile.sessionKey, sessionKey))
      .limit(1);
    ...
    if (existing.length) {
      await db.update(ZimaOSAgentProfile).set(payload).where(eq(ZimaOSAgentProfile.sessionKey, sessionKey));
    } else {
      await db.insert(ZimaOSAgentProfile).values(payload);
    }
