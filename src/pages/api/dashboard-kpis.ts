...
export const GET: APIRoute = async () => {
  ...
  try {
    const { db, Project, AgentTask, Request, AgentAppIssue, AgentDependencyRequest, eq, gte, inArray, count } =
      await loadAstroDb();
    ...
    const [
      ...
      [{ count: openAppIssuesCount }],
      [{ count: openDependencyRequestsCount }],
    ] = await Promise.all([
      db.select({ count: count() }).from(Project),
      db.select({ count: count() }).from(AgentTask),
      db.select({ count: count() }).from(AgentTask).where(gte(AgentTask.createdAt, today as any)),
    ...
    base.projectCount = projectsCount;
    base.tasksTotal = tasksTotalCount;
