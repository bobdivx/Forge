...
export default function OllamaTab() {
  ...
  const toggleManualDisable = async (
    model: string,
      ...
      <div class="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p class="text-xs text-gray-500">
            Gérez vos différents serveurs Ollama. Forge agrégera les modèles de
            toutes les instances actives pour votre matrice d'agents.
          </p>
        </div>
          ...
          >
            {batchTesting ? (
              <>
                ...
                Test : {batchProgress.modelName} ({batchProgress.current}/
                {batchProgress.total})
              </>
            ) : (
              <>
        ...
        {loading ? (
          ...
          <div class="grid gap-3">
            {instances.map((inst) => (
              <div
                key={inst.id}
                class={`p-4 rounded-xl border transition-all ${inst.enabled ? "bg-white border-gray-200 shadow-sm" : "bg-gray-50 border-gray-100 opacity-60"}`}
              >
                {editingId === inst.id ? (
                  <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <FormField label="URL">
                        <input
                          ...
                          value={editUrl}
                          onInput={(e) =>
                            setEditUrl((e.target as HTMLInputElement).value)
                          }
                          class="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#175B37] font-mono"
                        />
                      </FormField>
                    ...
                    <div class="flex gap-2">
                      <button
                        onClick={saveEdit}
                        class="bg-[#175B37] text-white px-3 py-1.5 rounded-lg text-[11px] font-bold"
                      >
                        Enregistrer
                      </button>
                      <button
                        onClick={() => setEditingId(null)}
                        class="bg-gray-100 text-gray-600 px-3 py-1.5 rounded-lg text-[11px] font-bold"
                      >
                        Annuler
                      </button>
                    </div>
                  ...
                  <div class="flex items-center justify-between">
                    <div class="space-y-1">
                      <div class="flex items-center gap-2">
                        ...
                        {inst.apiKey && (
                          <span class="px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 text-[10px] font-bold uppercase tracking-wider border border-blue-100">
                            Auth
                          </span>
                        )}
                      </div>
                      ...
                      {inst.enabled ? (
                        <>
                          <p
                            class={`text-[10px] ${inst.health?.ok ? "text-emerald-700" : "text-rose-700"}`}
                          >
                            {inst.health?.ok
                          ...
                          {inst.health?.ok &&
                            inst.health.models &&
                            inst.health.models.length > 0 && (
                              <div class="flex flex-wrap gap-2 mt-3">
                                {inst.health.models.map((m) => {
                                  const comp = compatibility[m];
                                  const isTesting =
                                    batchTesting &&
                                    batchProgress.modelName === m;
                                  const isDisabled = comp?.disabledManually;

                                  return (
                                    <div
                                      key={m}
                                      class={`flex items-center gap-1.5 p-1.5 rounded-lg border transition-all ${isDisabled ? "bg-gray-100 border-gray-200 opacity-50" : "bg-gray-50 border-gray-200"}`}
                                    >
                                      <span
                                        class={`text-[10px] font-mono font-bold ${isDisabled ? "text-gray-400 line-through" : "text-gray-700"}`}
                                      >
                                        {m}
                                      </span>
                                      ...
                                      {comp && !isDisabled && (
                                        <span
                                          ...
                                          title={
                                            comp.testedAt
                                              ? `Testé le ${new Date(comp.testedAt).toLocaleString()}`
                                              : ""
                                          }
                                        >
                                      ...
                                      {isDisabled && (
                                        <span class="px-1.5 py-0.5 rounded bg-gray-200 text-gray-500 text-[9px] font-bold uppercase">
                                          Désactivé
                                        </span>
                                      )}
                                      ...
                                      <div class="flex items-center gap-1 ml-1">
                                        <button
                                          ...
                                          aria-label={`Lancer un test pour ${m}`}
                                        >
                                          <svg
                                            class="w-3 h-3"
                                            fill="none"
                                        ...
                                        <button
                                          onClick={() =>
                                            toggleManualDisable(m, !!isDisabled)
                                          }
                    ...
                    <div class="flex items-center gap-2">
                      <button
                        ...
                        class={`text-[10px] font-bold px-3 py-1 rounded-full border transition-colors ${inst.enabled ? "border-amber-200 text-amber-700 hover:bg-amber-50" : "border-green-200 text-green-700 hover:bg-green-50"}`}
                      >
                        {inst.enabled ? "Désactiver" : "Activer"}
                      </button>
                      <button
                        onClick={() => startEdit(inst)}
                        class="p-2 text-gray-400 hover:text-gray-600 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#175B37] rounded-lg"
                        aria-label="Modifier l'instance"
                      ...
                      <button
                        ...
                        class="p-2 text-gray-400 hover:text-red-600 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#175B37] rounded-lg"
