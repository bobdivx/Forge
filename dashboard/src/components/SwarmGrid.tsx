...
export default function SwarmGrid() {
  ...
  return (
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {error && (
        <p class="col-span-full text-sm text-amber-400/90" role="alert">
          {error}
        </p>
      )}
      {agents.map((agent) => (
        ...
        >
          <div class="flex justify-between items-start mb-4">
            ...
            <div
              class={
                'badge badge-sm font-bold uppercase tracking-wider text-[9px] ' +
                (agent.status === 'actif' ? 'badge-success' : 'badge-ghost')
              }
            >
              {agent.status}
            </div>
