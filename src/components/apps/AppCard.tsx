...
export default function AppCard({ project, metrics, activeAgentId }: AppCardProps) {
  ...
  return (
    <div class="group flex flex-col overflow-hidden rounded-[1.5rem] border border-gray-100 bg-white shadow-sm transition-all duration-200 hover:border-gray-200 hover:shadow-md">
      <div class="p-5 flex-1">
        <div class="flex items-start justify-between mb-4">
          <div>
            ...
            <div class="flex items-center gap-2 mt-1">
              <span class={`w-2 h-2 rounded-full ${project.status === 'active' ? 'bg-emerald-500' : 'bg-gray-300'}`}></span>
              <span class="text-[10px] text-gray-500 uppercase tracking-wide">{project.status}</span>
            </div>
          </div>
          <a href={`/apps/by-id/${project.id}`} class="text-[#175B37] bg-[#E9F3EB] hover:bg-[#dceee0] p-2 rounded-full transition-colors">
        ...
        <div class="flex gap-3 mb-4">
          <div class="flex-1 bg-gray-50 rounded-xl p-3 border border-gray-100 flex items-center justify-between">
            ...
            <span class="font-bold text-gray-900">{metrics.pendingRequests}</span>
          </div>
          <div class="flex-1 bg-red-50/50 rounded-xl p-3 border border-red-100/50 flex items-center justify-between">
            ...
            <span class={`font-bold ${metrics.openIssues > 0 ? 'text-red-600' : 'text-gray-400'}`}>
              {metrics.openIssues}
            </span>
          </div>
        </div>
      ...
      <div class={`border-t px-5 py-3 flex items-center gap-3 ${isWorking ? 'bg-[#E9F3EB] border-[#175B37]/10' : 'bg-gray-50 border-gray-100'}`}>
        {isWorking ? (
          <>
          ...
          <>
            ...
            <span class="text-xs font-medium text-gray-500">En attente (Aucun agent)</span>
          </>
        )}
      </div>
    </div>
