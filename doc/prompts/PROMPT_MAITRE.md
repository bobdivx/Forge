...
- Les agents ne parlent pas à SQLite : ils appellent **forge-hook** (`doc/FORGE_API_CONTRACT.md`). Depuis ZimaOS en Docker avec Forge conteneurisé (`forge.yml`) : exporter **`FORGE_HOOK_BASE_URL=http://forge-host:4331`** puis utiliser **`scripts/forge-hook.sh`** avec **`source scripts/forge_env.sh`** (pas `127.0.0.1` depuis le conteneur).
