...
import { runZimaosIntegrationScan } from '../../lib/audit-zimaos-integration-scan';
import {
  FORGE_FACADES_OVER_ZIMAOS,
  FORGE_NATIVE_ROLES,
  ZIMAOS_RUNTIME_ROLES,
} from '../../lib/zimaos-integration-boundary';
  ...
  return new Response(
    JSON.stringify({
      ...
      reviewQueue: {
        ...
        hint:
          'Les composants UI ne doivent pas importer `zimaos-gateway` directement — utiliser les routes `/api/*` ou les façades `lib/forge-*`. Tout se pilote depuis Forge.',
      },
    }),
