...
async function invokeOllamaDirect(message: string, modelHint?: string): Promise<{
  ...
  async function resolveAvailableModel(origin: string, preferred: string): Promise<string> {
    try {
      ...
      const byName = new Map(selectable.map((m) => [m.name.toLowerCase(), m.name]));
      const exact = byName.get(preferred.toLowerCase());
      if (exact) return exact;
      for (const c of ['qwen2.5:7b', 'gemma4:latest', 'qwen3-coder:30b', 'llama3.2:latest']) {
        const match = byName.get(c.toLowerCase());
        if (match) return match;
      }
      if (selectable[0]?.name) return selectable[0].name;
