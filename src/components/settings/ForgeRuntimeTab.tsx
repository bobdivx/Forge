...
import ForgeAuditPanel from './ForgeAuditPanel';
...
export default function ForgeRuntimeTab({ settings, setSettings, onSave, saving, message }: Props) {
  ...
  const testSSH = async () => {
    ...
    setSshResult(null);
    try {
      const res = await fetch('/api/forge-ssh-test', {
        method: 'POST',
  ...
  const provisionSshKey = async () => {
    ...
    try {
      const res = await fetch('/api/forge-ssh-provision-key', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          zimaosHost: settings.zimaosHost,
