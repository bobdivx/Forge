...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    try {
      ...
      // Docker logs to either stdout or stderr depending on how the container logs
      const output = stdout + (stderr || "");
      logs = output.trim().split("\n");
    } catch (err: any) {
      // Some logs output on stderr, if command "failed" but stderr has content, we use it.
      const output =
