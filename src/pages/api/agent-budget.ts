...
export async function GET({ url }: { url: URL }) {
  try {
    ...
    const { db, AgentBudget, CostEvent, AgentInstruction, eq } = await loadAstroDb();
    ...
    const totalByAgent: Record<string, { cents: number; inputTokens: number; outputTokens: number; calls: number }> = {};
    for (const ev of allEvents) {
      if (!totalByAgent[ev.agentId]) {
        totalByAgent[ev.agentId] = { cents: 0, inputTokens: 0, outputTokens: 0, calls: 0 };
      }
    ...
    const monthlyByAgent: Record<string, { cents: number; inputTokens: number; outputTokens: number }> = {};
    for (const ev of allEvents) {
      const evDate = new Date(ev.occurredAt);
      if (evDate >= monthStart) {
        if (!monthlyByAgent[ev.agentId]) {
          monthlyByAgent[ev.agentId] = { cents: 0, inputTokens: 0, outputTokens: 0 };
        }
    ...
    if (agentId) {
      ...
      const monthly = monthlyByAgent[agentId] ?? { cents: 0, inputTokens: 0, outputTokens: 0 };
      const total = totalByAgent[agentId] ?? { cents: 0, inputTokens: 0, outputTokens: 0, calls: 0 };
      const pct = budget.monthlyCents > 0
        ? Math.round((monthly.cents / budget.monthlyCents) * 100)
        : null;

      return new Response(
        JSON.stringify({ ...budget, monthly, total, pct, model: agentModels[agentId] }),
    ...
    const result = [...allAgentIds].map((aid) => {
      ...
      const monthly = monthlyByAgent[aid] ?? { cents: 0, inputTokens: 0, outputTokens: 0 };
      const total = totalByAgent[aid] ?? { cents: 0, inputTokens: 0, outputTokens: 0, calls: 0 };
      const pct = budget.monthlyCents > 0
        ? Math.round((monthly.cents / budget.monthlyCents) * 100)
        : null;
    ...
    const globalMonthly = Object.values(monthlyByAgent).reduce(
      (acc, v) => ({ cents: acc.cents + v.cents, inputTokens: acc.inputTokens + v.inputTokens, outputTokens: acc.outputTokens + v.outputTokens }),
      { cents: 0, inputTokens: 0, outputTokens: 0 }
    );

    return new Response(
      JSON.stringify({ agents: result, globalMonthly }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  } catch (err: unknown) {
...
export async function PATCH({ request }: { request: Request }) {
  ...
  const { agentId, monthlyCents, alertThreshold, hardStop, resetSpent } = body as {
    agentId?: string;
    monthlyCents?: number;
  ...
  try {
    ...
    if (existing.length === 0) {
      ...
      await db.update(AgentBudget).set(updates).where(eq(AgentBudget.agentId, agentId));
    }
