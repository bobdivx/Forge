...
export default function AgentDirectiveForm({ sessionKey }: { sessionKey: string }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setMsg({
          type: 'err',
          text: typeof data.error === 'string' ? data.error : 'Envoi refusé par le gateway',
        });
        return;
      }
  ...
  return (
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
      <h3 class="font-bold text-white text-sm uppercase tracking-widest text-slate-500">
        Contrôle session
      </h3>
      ...
      <button
        ...
        class={`btn btn-primary btn-sm w-full gap-2 shadow-lg shadow-blue-500/20 ${loading ? 'loading' : ''}`}
      >
        {!loading && (
