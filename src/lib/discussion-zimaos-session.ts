/**
 * Résolution de session ZimaOS + extraction de messages pour la page Discussion
 * (poll en direct + historique au chargement).
 */
import { fetchZimaOSSessionsPayload, normalizeZimaOSSessions } from './zimaos-gateway';

function toUpper(v: unknown): string {
  return String(v ?? '').trim().toUpperCase();
}

export function sessionMatchesZimaOS(raw: Record<string, unknown>, sessionKey: string): boolean {
...
export function getZimaOSSessionMessagesArray(raw: Record<string, unknown>): unknown[] {
...
function sessionScore(raw: Record<string, unknown>, sessionKey: string): number {
  ...
  const msgArr = getZimaOSSessionMessagesArray(raw);
  if (msgArr.length > 0) score += 15;
  return score;
}

export function resolveBestZimaOSSessionForKey(
  sessions: Record<string, unknown>[],
  sessionKey: string,
): Record<string, unknown> | null {
  const candidates = sessions.filter((s) => sessionMatchesZimaOS(s, sessionKey));
...
export function parseZimaOSMessageTimestampMs(o: Record<string, unknown>): number {
  const n =
...
export function extractZimaOSMessageText(o: Record<string, unknown>): string {
...
export function pickLatestAssistantMessage(
  ...
  const msgs = getZimaOSSessionMessagesArray(raw);
  ...
  for (let i = msgs.length - 1; i >= 0; i -= 1) {
    ...
    const text = extractZimaOSMessageText(o);
    if (!text) continue;
    const tsMs = parseZimaOSMessageTimestampMs(o);
...
/**
 * Construit l’historique chat (user + assistant) dans l’ordre ZimaOS, pour réhydrater l’UI après F5.
 */
export function buildZimaOSDiscussionHistory(
  raw: Record<string, unknown>,
  opts?: { max?: number },
  ...
  for (const m of msgs) {
    ...
    const parsedMs = parseZimaOSMessageTimestampMs(o);
...
export async function fetchZimaOSSessionsForDiscussion(
  email: string | undefined,
  messageLimit = 120,
): Promise<FetchDiscussionSessionsResult> {
  const payload = await fetchZimaOSSessionsPayload(email, {
    invokeOnly: true,
