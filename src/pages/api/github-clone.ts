...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (String(repoUrl).trim().startsWith('-') || String(repoName).trim().startsWith('-')) {
      return new Response(JSON.stringify({ error: 'Paramètres invalides (injection suspectée)' }), { status: 400 });
    }
    ...
    const errorMsg = error.message || 'Erreur lors du clonage';
    // Remove the githubToken if it was leaked in the command error output
    const safeErrorMsg = errorMsg.replace(/oauth2:[^@]+@/g, 'oauth2:***@');
