...
export const GET: APIRoute = async ({ locals }) => {
  ...
  // Initiate slow network call early and attach dummy catch to prevent unhandled rejection
  const sessionsPromise = fetchZimaOSSessionsPayload(email);
  ...
  try {
    ...
    for (const projectPromise of projectPromises) {
      payload.projects.push(await projectPromise);
    }
  ...
  try {
    const oc = await sessionsPromise;
