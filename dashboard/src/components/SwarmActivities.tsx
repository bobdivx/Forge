...
export default function SwarmActivities() {
    ...
    fetch('/api/openclaw-swarm-stats')
      ...
      .then((d) => {
        ...
        setStats({
          ...
          idleCount: d.idleCount ?? 0,
          totalSessions: d.totalSessions ?? 0,
          uniqueAgents: d.uniqueAgents ?? 0,
        });
        setLoading(false);
      })
      .catch(() => {
  ...
  const dotClass = (tone: string) => {
    if (tone === 'blue') return 'bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.8)]';
  ...
  return (
    <>
      <div class="bg-slate-900/50 border border-slate-800 rounded-xl p-6 backdrop-blur-sm">
        <h3 class="font-bold text-white mb-6 border-b border-slate-800 pb-3 uppercase tracking-widest text-xs text-slate-500">
          Activité récente (sessions OpenClaw)
        </h3>
        ...
        {loading ? (
          ...
          <div class="space-y-6">
            {activities.map((a, i) => (
              <div
                class={`flex gap-4 group ${i > 0 ? 'border-t border-slate-800/50 pt-6' : ''}`}
                key={`${a.title}-${i}`}
              >
                <div class={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${dotClass(a.tone)}`}></div>
                <div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="bg-slate-900 border border-slate-800 p-4 rounded-xl">
          ...
          <div class="text-3xl font-bold text-white mt-1">
            {stats.totalSessions}
            <span class="text-sm font-normal text-slate-500 ml-2">{stats.uniqueAgents} agent(s) distincts</span>
          </div>
        </div>
      </div>
    </>
