#!/usr/bin/env bash
# Diagnostic réseau Ollama pour ZimaOS (à lancer sur le NAS ou depuis une machine qui voit les deux cibles).
set -euo pipefail
...
echo "=== 3) Rappels ==="
echo "- ZimaOS en conteneur Linux : baseUrl NAS souvent http://172.17.0.1:11434 (pont Docker → hôte)."
echo "- Si Ollama NAS est publié sur un autre port hôte (ex. 38197), remets ce port dans zimaos.json."
...
echo "- Après édition zimaos.json : redémarrer le gateway ZimaOS."
