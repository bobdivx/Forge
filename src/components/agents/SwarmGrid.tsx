...
function formatAgentName(name: string) {
  ...
  return name.replace('telegram:g-agent-', '').replace('agent:', '').replace(':main', '').toUpperCase();
}
...
export default function SwarmGrid() {
  ...
  if (loading && agents.length === 0) {
    return (
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => <div key={i} class="bg-slate-900 border border-slate-800 rounded-2xl p-6 h-48 animate-pulse" />)}
      </div>
    );
  }
  ...
  return (
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {error && <p class="col-span-full text-sm text-amber-400/90">{error}</p>}
      {agents.map((agent) => (
        <a href={'/swarm/' + encodeURIComponent(agent.id)} key={agent.id} class="group bg-slate-900 border border-slate-800 rounded-2xl p-6 hover:border-blue-500/50 hover:bg-slate-800/50 transition-all duration-300 shadow-xl flex flex-col">
          <div class="flex justify-between items-start mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-bold text-white group-hover:text-blue-400 transition-colors truncate max-w-[150px]">{formatAgentName(agent.name)}</h3>
                <span class="text-[10px] font-mono text-slate-500">{agent.id.split(':').pop()?.slice(0, 8)}</span>
              </div>
            </div>
            <div class={'badge badge-sm font-bold uppercase tracking-wider text-[9px] ' + (agent.status === 'actif' ? 'badge-success' : 'badge-ghost')}>
              {agent.status}
            </div>
          </div>
