...
export default function OpenClawModelsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const r = await fetch('/api/openclaw-models');
  ...
  async function ping(agentId: string, openAiTarget: string) {
    ...
    try {
      const r = await fetch('/api/openclaw-model-ping', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  if (loading && !data) {
    return (
      <div class="text-slate-400 text-sm animate-pulse">Chargement des modèles OpenClaw…</div>
    );
  }
  ...
  return (
    <div class="space-y-8 max-w-7xl mx-auto">
      <div>
        <h2 class="text-2xl font-bold tracking-tight text-white">IA &amp; modèles OpenClaw</h2>
        <p class="mt-1 text-sm text-slate-400">
          <code class="text-slate-500 font-mono text-xs">GET /v1/models</code> expose les{' '}
          <strong class="text-slate-300">cibles agent</strong> au format OpenAI (<code class="text-slate-500 font-mono text-xs">openclaw/…</code>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="rounded-xl border border-cyan-500/20 bg-slate-900/80 p-4">
          <p class="text-[11px] uppercase tracking-wide text-slate-500">Cibles openclaw/…</p>
                  ...
                  <tr key={row.agentId} class="border-b border-slate-800/80 align-top">
                    ...
                    <td class="py-3 pr-3">
                      <div class="flex flex-wrap gap-1">
                        <Badge ok={row.inGatewayRegistry} label="registre" title="Présent dans agents_list OpenClaw" />
